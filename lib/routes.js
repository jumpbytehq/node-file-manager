...
router.get('/views/:filename', function *() {
	C.logger.info(this.request)
  this.body = yield render('view');
});

router.get('/views/api/(.*)', Tools.loadRealPath, Tools.checkPathExists, function *() {
  var p = this.request.fPath;
  this.body = yield * FileManager.readFile(p);
});
