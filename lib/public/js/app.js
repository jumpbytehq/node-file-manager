...
FMApp.controller('FileManagerCtr', ['$scope', '$http', '$location',
  function ($scope, $http, $location) {
    ...
    var downloadFile = function (file) {
      $http.get('api' + file.relPath ,{params: {type: 'ReadFile'}},{headers: {'Content-Type': 'text/plain'}})
        .success(function (data) {
          let pdfWindow = window.open("", "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=100,left=50,width=1000,height=800");
          pdfWindow.document.write("<pre>" + data + "</pre>");
        })
        .error(function (data, status) {
          alert('Error: ' + status + data);
        });
